...
#-------------------------------------------------------------------------------
  def train(self, x_train, y_train, x_test, y_test, epochs, datagen, batch_size, lr, trigger):
      # trigger_ag = np.array([100, 250, 300, 400])
      trigger_ag = np.array([4, 250, 300, 400])
      history = {
          'val_loss':[],
          'train_loss':[],
          'val_acc':[],
      ...
      for e in range(0, epochs):
          ...
          if np.isfinite(val_loss):
              ...
              if self.num_features == 28*28:
                ...
                precision_score_val = precision_score(val_preds, y_test_aux)
                history['val_precision'].append(precision_score_val)
                print('Val_precision:',precision_score_val,'Val acc:', val_acc)
          if flag2 == 0:
            flag2 = 1
            if  trigger<0.1: 
              print('AG trigger') 
...
#-------------------------------------------------------------------------------    
  def pop_initial(self):
    n_pop = int(np.ceil((self.num_max_layers * 50) / (5 + self.num_max_layers)))
    ...
    self.n_pop = len(self.pop_layers)
#-------------------------------------------------------------------------------
  def fitness(self, epochs):
    ...
    for i in range(np.max([self.n_pop, len(self.pop_layers)])):
      ...
      net = self.main_fluid_net.copy_actual_instance() 
      ...
      # Training parameters
      (x_train, y_train), (x_test, y_test) = load_data_patterns()
      batch_size = 128
      steps_per_epoch = int(x_train.shape[0]/batch_size)
      lr = 1e-2
...
#-------------------------------------------------------------------------------
  def crossover(self, crossover_rate, nchild): # Two children by each recombination
    ...
    if crossover_rate > threshold: # Then crossover operator is computed
      ...
      c = self.permutation2(nchild) # number of children
...
#-------------------------------------------------------------------------------
  def mutation(self, mutation_rate):
    ...
    if threshold < mutation_rate:
        n  = np.random.randint(low=self.num_min_layers, high=self.num_max_layers, size=n_mut_pop)
        for i in range(n_mut_pop):
          c[i] = np.random.randint(low=self.num_min_units, high=self.num_max_units, size=n[i])
        return c
    else:
       return []
#-------------------------------------------------------------------------------
  def generational_replacement(self): 
    ...
    while len(next_pop) < self.n_pop :
      aux1 = self.crossover(0.8, 1)
      aux2 = self.mutation(0.3)
      if len(aux1) != 0:
      ...
      if len(aux2) != 0:
          aux2_ = list(aux2.values())[0]
          next_pop.append(aux2_)
      i +=1
  ...
  def run_algo(self, n_stall_generations, max_iters):
     ...
     while stall_generations < n_stall_generations or iters < max_iters:
       ...
       if self.best_fitness < max(pop_fitness):
         ...
         stall_generations = 0
         print(f'Iter {iters}.The best NN has a precision of {self.best_fitness }')
       else:
         stall_generations += 1
